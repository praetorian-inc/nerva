...
var findingCatalog = map[string]FindingMeta{
	...
	"rdp-eol-os": {
		...
		Severity:       SeverityLow,
		Impact:         "The host is running an operating system that no longer receives security patches, leaving all known and future vulnerabilities permanently unpatched.",
		Recommendation: "Migrate the workload to a supported operating system version. If immediate migration is not possible, isolate the host on a restricted network segment and apply compensating controls.",
		CVSS:           "CVSS:3.1/AV:N/AC:H/PR:N/UI:N/S:U/C:L/I:N/A:N",
	},
	...
	"s7comm-read-only": {
		...
		Severity:       SeverityLow,
		Impact:         "Read-only S7comm access allows attackers to enumerate PLC configuration, firmware version, and process data without modifying control logic.",
		Recommendation: "Enable S7comm access protection to restrict read access. Isolate PLCs on dedicated OT networks and monitor for unauthorized queries.",
		CVSS:           "CVSS:3.1/AV:N/AC:H/PR:N/UI:N/S:U/C:L/I:N/A:N",
	},
	"modbus-no-auth": {
