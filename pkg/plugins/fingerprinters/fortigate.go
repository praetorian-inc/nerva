...
// FortiGate body detection patterns
var fortiGateBodyPatterns = []*regexp.Regexp{
	regexp.MustCompile(`(?i)/remote/login`),          // SSL VPN login redirect
	regexp.MustCompile(`(?i)ftnt-fortinet-grid`),     // Fortinet icon class
	regexp.MustCompile(`(?i)/remote/fgt_lang`),       // FortiGate language file
}
...
func (f *FortiGateFingerprinter) Match(resp *http.Response) bool {
	...
	if fortiGateETagPattern.MatchString(etag) {
		return true
	}
