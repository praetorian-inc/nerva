...
// TestSUAPlugin_Run_ValidASPUpAck tests detection with valid ASP Up Ack
func TestSUAPlugin_Run_ValidASPUpAck(t *testing.T) {
	...
	assert.Equal(t, SUA_SCTP, service.Protocol)
	assert.Equal(t, "test.sua.local", service.Host)

	// Verify metadata
	assert.NotEmpty(t, service.Raw)
}

// TestSUAPlugin_Run_ErrorResponse tests handling of error responses
func TestSUAPlugin_Run_ErrorResponse(t *testing.T) {
...
// TestSUAPlugin_Run_InvalidResponse tests rejection of invalid responses
func TestSUAPlugin_Run_InvalidResponse(t *testing.T) {
...
// TestSUAPlugin_PortPriority tests default port recognition (both 14001 and 2904)
func TestSUAPlugin_PortPriority(t *testing.T) {
	plugin := &SUAPlugin{}

	assert.True(t, plugin.PortPriority(14001), "IANA port 14001 should be priority")
	assert.True(t, plugin.PortPriority(2904), "field-common port 2904 should be priority")
	assert.False(t, plugin.PortPriority(2905))
	assert.False(t, plugin.PortPriority(80))
}

// TestSUAPlugin_Name tests plugin name
func TestSUAPlugin_Name(t *testing.T) {
	plugin := &SUAPlugin{}
	assert.Equal(t, SUA_SCTP, plugin.Name())
}

// TestSUAPlugin_Type tests protocol type
func TestSUAPlugin_Type(t *testing.T) {
	plugin := &SUAPlugin{}
	assert.Equal(t, plugins.SCTP, plugin.Type())
}

// TestSUAPlugin_Priority tests plugin priority
func TestSUAPlugin_Priority(t *testing.T) {
	plugin := &SUAPlugin{}
...
func (m *mockConn) RemoteAddr() net.Addr {
	return &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: 14001}
}
