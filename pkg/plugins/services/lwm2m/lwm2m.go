...
type LwM2MPlugin struct{}

func init() {
	plugins.RegisterPlugin(&LwM2MPlugin{})
}

func (p *LwM2MPlugin) Run(conn net.Conn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	// Generate random 2-byte Message ID
	msgID := make([]byte, 2)
	if _, err := rand.Read(msgID); err != nil {
		return nil, &utils.RandomizeError{Message: "lwm2m message id"}
	}
	...
	payload := extractLwM2MPayload(response)
	if payload == nil {
		return nil, nil
	}
...
// extractLwM2MPayload walks the CoAP response, extracts the CoRE Link Format
// payload, and checks for LwM2M-specific resources. Returns nil if the payload
// does not contain the required /rd resource.
func extractLwM2MPayload(response []byte) *plugins.ServiceLwM2M {
	if len(response) < 4 {
		return nil
	}
	...
	// LwM2M requires /rd (Registration Directory) resource
	if !strings.Contains(payloadStr, "</rd>") {
		return nil
	}

	result := &plugins.ServiceLwM2M{
		HasRegistration: true,
		HasBootstrap:    strings.Contains(payloadStr, "</bs>"),
		Resources:       payloadStr,
	}

	// Detect Californium (Leshan's default CoAP stack) via "Cf " prefix
	if strings.Contains(payloadStr, "Cf ") {
		result.ServerImpl = "leshan"
	} else {
		result.ServerImpl = "unknown"
	}

	return result
}

func (p *LwM2MPlugin) PortPriority(i uint16) bool { return i == 5683 || i == 5783 }
func (p *LwM2MPlugin) Name() string               { return LwM2M }
func (p *LwM2MPlugin) Type() plugins.Protocol     { return plugins.UDP }
func (p *LwM2MPlugin) Priority() int              { return 1999 }
