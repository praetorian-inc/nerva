...
// TestM3UAPlugin_Run_ValidASPUpAck tests detection with valid ASP Up Ack
func TestM3UAPlugin_Run_ValidASPUpAck(t *testing.T) {
	...
	assert.Equal(t, M3UA_SCTP, service.Protocol)
	assert.Equal(t, "test.m3ua.local", service.Host)

	// Verify metadata
	assert.NotEmpty(t, service.Raw)
}

// TestM3UAPlugin_Run_ErrorResponse tests handling of error responses
func TestM3UAPlugin_Run_ErrorResponse(t *testing.T) {
...
// TestM3UAPlugin_Run_InvalidResponse tests rejection of invalid responses
func TestM3UAPlugin_Run_InvalidResponse(t *testing.T) {
...
// TestM3UAPlugin_PortPriority tests default port recognition
func TestM3UAPlugin_PortPriority(t *testing.T) {
	plugin := &M3UAPlugin{}

	assert.True(t, plugin.PortPriority(2905))
	assert.False(t, plugin.PortPriority(3868))
	assert.False(t, plugin.PortPriority(80))
}

// TestM3UAPlugin_Name tests plugin name
func TestM3UAPlugin_Name(t *testing.T) {
	plugin := &M3UAPlugin{}
	assert.Equal(t, M3UA_SCTP, plugin.Name())
}

// TestM3UAPlugin_Type tests protocol type
func TestM3UAPlugin_Type(t *testing.T) {
	plugin := &M3UAPlugin{}
	assert.Equal(t, plugins.SCTP, plugin.Type())
}

// TestM3UAPlugin_Priority tests plugin priority
func TestM3UAPlugin_Priority(t *testing.T) {
	plugin := &M3UAPlugin{}
...
func (m *mockConn) RemoteAddr() net.Addr {
	return &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: 2905}
}
