...
/*
M3UA (MTP3 User Adaptation Layer) Protocol Fingerprinting over SCTP

M3UA is part of the SIGTRAN protocol suite for transporting SS7 signaling over IP using SCTP.
This is a CRITICAL protocol for telecom security reconnaissance.

Protocol Specification:
  RFC 4666 (obsoletes RFC 3332)
  Transport: SCTP (IP protocol 132)
  Default Port: 2905
  SCTP PPID: 3 (definitive identification)
...
M3UA-Specific Message Classes:
  - Class 0: Management (MGMT) - Error, Notify
  - Class 1: Transfer Messages - DATA
  - Class 2: SSNM - DUNA, DAVA, DAUD, SCON, DUPU, DRST
  - Class 3: ASPSM - ASP Up/Down, Heartbeat (all SIGTRAN)
  - Class 4: ASPTM - ASP Active/Inactive (all SIGTRAN)
  - Class 9: RKM - Registration Request/Response

Detection Strategy:
  1. Primary: Check SCTP PPID = 3 (definitive)
  2. Fallback: Port 2905 when PPID = 0 (misconfiguration)
...
type M3UAPlugin struct{}

func init() {
	plugins.RegisterPlugin(&M3UAPlugin{})
}

// Run implements the main fingerprinting logic
func (p *M3UAPlugin) Run(conn net.Conn, timeout time.Duration, target plugins.Target) (*plugins.Service, error) {
	// Phase 1: Detection - send ASP Up and receive ASP Up Ack or Error
	response, err := detectM3UA(conn, timeout)
	if err != nil {
		return nil, err
	}

	// Phase 2: Enrichment - extract metadata from response
	messageClass, messageType, errorCode, infoString, err := enrichM3UA(response)
	if err != nil {
		// Detection succeeded but enrichment failed - still return service
		metadata := ServiceM3UA{
...
// PortPriority returns true if the port is 2905 (default M3UA port)
func (p *M3UAPlugin) PortPriority(port uint16) bool {
	return port == M3UA_PORT
}
...
// M3UA uses port 2905, run at same priority as Diameter (60)
func (p *M3UAPlugin) Priority() int {
	return 60
}

// detectM3UA sends an ASP Up message and validates the response
func detectM3UA(conn net.Conn, timeout time.Duration) ([]byte, error) {
...
func validateASPUpAck(response []byte) error {
	...
	if len(response) < HEADER_LENGTH {
		return &utils.InvalidResponseErrorInfo{
			Service: M3UA_SCTP,
			Info:    fmt.Sprintf("response too short for valid M3UA message: got %d bytes, need %d", len(response), HEADER_LENGTH),
		}
	}

	// Check version (byte 0)
	if response[0] != M3UA_VERSION {
		return &utils.InvalidResponseErrorInfo{
			Service: M3UA_SCTP,
			Info:    fmt.Sprintf("invalid version: %d, expected %d", response[0], M3UA_VERSION),
		}
	}
...
// enrichM3UA extracts metadata from the response
// Returns: messageClass, messageType, errorCode, infoString, error
func enrichM3UA(response []byte) (uint8, uint8, uint32, string, error) {
	if len(response) < HEADER_LENGTH {
...
func (s ServiceM3UA) Type() string {
	return M3UA_SCTP
}
