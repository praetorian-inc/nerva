...
func (e Service) Metadata() Metadata {
	...
	case ProtoOPCUA:
		...
		_ = json.Unmarshal(e.Raw, &p)
		return p
	case ProtoOpenVPNManagement:
		var p ServiceOpenVPNManagement
		_ = json.Unmarshal(e.Raw, &p)
		return p
	case ProtoPCOM:
		var p ServicePCOM
...
// ServiceOracleCoherence identifies an Oracle Coherence data-grid node
// (best-effort binary detection on the 7574 cluster/NameService port). No
// version is exposed on the wire, so CPE is versionless.
...
func (e ServiceOpenVPNManagement) Type() string { return ProtoOpenVPNManagement }
